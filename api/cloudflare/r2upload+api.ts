...
async function uploadMultipartToR2(file: File) {
  try {
    const arrayBuffer = await file.arrayBuffer();
    const buffer = Buffer.from(arrayBuffer);
    const key = `${process.env.CLOUDFLARE_R2_S3_BUCKET_NAME}/${file.name}`;
...
function getUploadMethod(size: number): 'multipart' | 'direct' | 'worker' {
  console.log('Getting upload method for file size:', size);
  if (size > 5 * 1024 * 1024 * 1024) return 'multipart';
  if (size > 100 * 1024 * 1024) return 'direct';
  return 'worker';
}
